...
answer problem = getFirstNum fields
  where
    ...
    allowedOp = ["plus", "minus", "multiplied", "divided", "cubed"]
    fields = filter isFieldOk $ splitWhen isSpace cleaned
      where
        isFieldOk s = all (\c -> isDigit c || c == '-') s || s `elem` allowedOp
    ...
    calculate left op right = case op of
       ...
       "divided"    -> if right /= 0 then Just $ left `quot` right else Nothing
       _            -> Nothing
